...
public class WebSocketTransport extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof CloseWebSocketFrame) {
            ...
            // todo 如果升级为websocket，以后会用websocket解码，触发InPacketHandler进行处理
            // todo socketio是在websocket的基础上，自定义了协议body的内容？自定义了ping/pong/message event等？
            ...
            boolean upgrade = transport != null && NAME.equals(transport.get(0));
            log.info("是否升级websocket：{}", upgrade);
            if (upgrade) {
                try {
