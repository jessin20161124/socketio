...
public class ClientHead {
    ...
    public void bindChannel(Channel channel, Transport transport) {
        log.info("绑定通道: {} to transport: {}", channel, transport);
    ...
    public void schedulePingTimeout() {
        ...
        disconnectScheduler.schedule(key, new Runnable() {
            ...
            public void run() {
                ...
                if (client != null) {
                    // todo 超过pingTimeOut + pingInterval时间没有收到浏览器心跳，则关闭连接？？？
    ...
    public ChannelFuture send(Packet packet, Transport transport) {
        ...
        if (channel == null
            ...
            log.info("发送包：{}，但是channel为空:{}", packet, channel);
            return null;
        }
        // clientHead只写一次，底层通过队列获取到后续的所有数据
        return sendPackets(transport, channel);
    }

    /**
     * todo 这里会触发拦截器链，进入写编码处理器EncoderHandler
     * @param transport
     * @param channel
     * @return
     */
